...
from src.models import User, Users, UserCreate
...
@router.post('/login')
async def login_user(session: SessionDep, credentials: OAuth2PasswordRequestForm = Depends()):
...
async def delete_user(session: SessionDep, target: User):
    result = await session.execute(select(Users).where(Users.username == target.username))
